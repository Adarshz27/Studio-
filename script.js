...
const showMenuPage = (id, updateUrl = true) => {
  const target = document.getElementById(id);
  if (!target || !target.classList.contains("menu-page-section")) return false;

  // Make the landing-page sections disappear before showing the selected page.
  document.body.classList.add("single-menu-page");
  hideAllMenuPages();
  target.classList.add("active-menu-page");

  // Put the user at the beginning of the selected page.
  window.scrollTo(0, 0);

  // Keep the URL useful without letting the browser jump to the hidden element.
  if (updateUrl) {
...
document.querySelectorAll(".nav a").forEach(a => a.addEventListener("click", e => {
  ...
  // Home, About and Reviews are intentionally part of the landing page.
  ...
  // Every other section is a standalone menu page.
  if (target.classList.contains("menu-page-section")) {
    showMenuPage(id);
  }
}));

// Buttons/links inside the site follow the same landing-vs-menu-page rule.
document.querySelectorAll('a[href^="#"]').forEach(a => {
  ...
  a.addEventListener("click", e => {
    ...
    if (target.classList.contains("landing-section")) {
      showLandingPage(id);
      history.replaceState({ page: id }, "", `#${id}`);
    } else if (target.classList.contains("menu-page-section")) {
      showMenuPage(id);
    }
  });
});

// Back/forward navigation between menu pages and landing sections.
window.addEventListener("popstate", () => {
  ...
  if (target?.classList.contains("menu-page-section")) {
    showMenuPage(id, false);
  } else if (target?.classList.contains("landing-section")) {
    showLandingPage(id);
  } else {
    showLandingPage();
  }
